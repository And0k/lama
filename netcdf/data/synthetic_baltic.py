...
HALO_DS:  tuple[float, float] = (4.0,  12.0)   # salinity jump [PSU]
HALO_TH:  tuple[float, float] = (3.0,  15.0)   # thickness [m]
...
class SceneGenerator:
    ...
    def generate(
        ...
        if cfg.cil_present:
            ...
            cil_amp  = self.rng.uniform(*cfg.cil_amp_range)   # negative = cold
            cil_x0   = self.rng.uniform(0, domain.xmax)
            cil_sx   = self.rng.uniform(*CIL_SX)
            cil_sz   = self.rng.uniform(*CIL_SZ)

            L_cil = np.array([[cil_x0, cil_z, cil_sx, cil_sz, cil_amp]])
            render_lenses(T, x, z, L_cil)

        # --- additional lenses (intrusions / anomalies) -----------------
        n_lens = self.rng.integers(*cfg.n_lens_range) + ov.get("extra_lenses", 0)
        if n_lens > 0:
            L = np.column_stack([
                self.rng.uniform(0,      domain.xmax, n_lens),   # x0
                self.rng.uniform(20,     150,          n_lens),   # z0
                self.rng.uniform(5_000,  40_000,       n_lens),   # sx
                self.rng.uniform(5,      30,            n_lens),   # sz
                self.rng.uniform(-8, 8,                n_lens)    # A (T)
                * ms.lens_amp_scale,
            ])
            render_lenses(T, x, z, L)
