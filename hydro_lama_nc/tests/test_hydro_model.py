...
def generator():
    ...
    return HydroGenerator(in_ch=7, base_ch=32, n_blocks=6, ratio_g=0.5)
...
# ── Test 5: Physics loss gradient flow ─────────────────────────────────

class TestPhysicsLossIntegration:

    def test_physics_is_scalar(self, generator, batch):
        from hydro_lama_nc.losses import physics_loss
        generator.train()
        pred = generator(batch["image"])
        loss = physics_loss(pred, mask_below=batch["mask"])
        assert loss.shape == torch.Size([]), "Physics loss is not scalar"

    def test_physics_loss_gradients(self, generator, batch):
        from hydro_lama_nc.losses import physics_loss
        generator.train()
        pred = generator(batch["image"])
        loss = physics_loss(pred, mask_below=batch["mask"])
        ...
        real_dead = [(n, g) for n, g in zero_grad if n not in known_dead]
        assert len(real_dead) == 0, f"Physics loss dead gradients: {real_dead}"
...
class TestFourierUnit:
    ...
    def test_output_is_real_tensor(self):
        from hydro_lama_nc.model import FourierUnit
        fu = FourierUnit(8)
        fu.eval()
...
class TestSpectralTransform:
    ...
    def test_square_input_preserved(self):
        from hydro_lama_nc.model import SpectralTransform
...
class TestFFCResBlock:
    ...
    def test_channel_split_sum(self):
        from hydro_lama_nc.model import FFCResBlock
...
class TestReceptiveField:
    ...
    def test_generator_coverage(self, generator):
        mean_cov = self._measure_rf(generator, (1, 7, 64, 64))
...
# ── Test 11: HydroGenerator accepts 7ch input, outputs 2ch ──────────────

class TestHydroGenerator:

    def test_hydrogenerator_alias(self):
        from hydro_lama_nc.model import HydroGenerator, LamaGenerator
        assert HydroGenerator is LamaGenerator

    def test_factory_import(self):
        from hydro_lama_nc.model import HydroGenerator
        gen = HydroGenerator(in_ch=7, base_ch=32, n_blocks=6, ratio_g=0.5)
...
class TestHydroLightningModule:
    ...
    def test_forward_with_batch_dict(self):
...
# ── Test 13: Physics loss with generated predictions ─────────────────────

class TestPhysicsWithPrediction:

    def test_physics_with_real_pred(self, batch):
        from hydro_lama_nc.losses import physics_loss
    ...
    def test_physics_differentiable(self, batch):
        from hydro_lama_nc.losses import physics_loss
