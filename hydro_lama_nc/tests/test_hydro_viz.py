...
class TestPlotInputChannels:
    def test_creates_png(self):
        from hydro_lama_nc.visualization import plot_input_channels
...
class TestPlotTwoFieldRows:
    def test_creates_png(self):
        from hydro_lama_nc.visualization import plot_two_field_rows
...
class TestPlotFieldsResult:
    def test_oi_and_lama_calls(self):
        from hydro_lama_nc.visualization import plot_fields_result
...
class TestPlotFieldsError:
    def test_creates_png(self):
        from hydro_lama_nc.visualization import plot_fields_error

        rng = np.random.default_rng(42)
        T_field = rng.normal(0.5, 0.1, (16, 16)).astype(np.float32)
        S_field = rng.normal(0.5, 0.1, (16, 16)).astype(np.float32)
        T_bg = rng.normal(0.5, 0.1, (16, 16)).astype(np.float32)
        S_bg = rng.normal(0.5, 0.1, (16, 16)).astype(np.float32)
        below = np.zeros((16, 16), dtype=bool)

        with tempfile.TemporaryDirectory() as tmpdir:
            plot_fields_error(T_field, S_field, T_bg, S_bg, method="lama",
                              below=below, output_dir=tmpdir, suffix="s1")
            assert os.path.exists(os.path.join(tmpdir, "lama_error_s1.png"))

            plot_fields_error(T_field * 0.5, S_field * 0.5, T_bg, S_bg, method="oi",
                              below=below, output_dir=tmpdir, suffix="s1")
...
class TestPlotNetcdfInferenceUpdated:
    def test_has_error_column(self):
        from hydro_lama_nc.visualization import plot_netcdf_inference
