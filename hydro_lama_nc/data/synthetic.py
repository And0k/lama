...
def synthetic_V_field(nx: int, nz: int, T_field: np.ndarray,
    ...
    """Generate velocity from geostrophic balance.

    In geostrophic balance, horizontal velocity shear is proportional
    to horizontal density (temperature) gradient with correct sign:
    ∂u/∂z ~ −∂ρ/∂x ~ α·∂T/∂x.  The signed gradient preserves the
    direction of the thermal wind relation.
...
def sample_observations(
    ...
    # Jitter CMEMS grid positions to prevent the model from memorising
    # fixed observation locations.  Each point shifts by ±half_spacing
    # in both x and z (clamped to valid range).
    if n_cmems_x > 1:
...
def make_hydro_synthetic_sample(
    ...
    from .ctd_dropout import _enforce_stratification
    ...
    # Enforce stratification only at non-CTD columns for the target.
    # CTD observations contain realistic noise-driven inversions.
    ctd_col_set = set(np.where(mask_ctd[0, :] > 0.5)[0])
    non_ctd_cols = np.array([x for x in range(nx) if x not in ctd_col_set])
    if non_ctd_cols.size > 0:
        T_enf, S_enf = _enforce_stratification(T, S, below)
        T_enf[:, list(ctd_col_set)] = T[:, list(ctd_col_set)]
        S_enf[:, list(ctd_col_set)] = S[:, list(ctd_col_set)]
        T, S = T_enf, S_enf
    ...
    return {
        ...
        "v_input": v_lr[None],         # (1, nz, nx) — meridional velocity input
