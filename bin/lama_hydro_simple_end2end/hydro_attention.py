...
class OceanDataset(Dataset):
    def init(self, n, cfg, seed=0):
        rng = np.random.default_rng(seed)
        self.samples = [make_sample(cfg, rng) for  in range(n)]

    def len(self): return len(self.samples)

    def getitem(self, i):
        s = self.samples[i]
        return (torch.fromnumpy(s['input']),
...
class LamaGenerator(nn.Module):
    ...
    """
    def init(self, in_ch=8, base_ch=32, n_blocks=6):
        super().init()
        ch = base_ch  2   # channels after encoder stride

        self.enc = nn.Sequential(
            nn.Conv2d(in_ch, base_ch, 7, padding=3), nn.ReLU(True),
            nn.Conv2d(base_ch, ch, 3, stride=2, padding=1), nn.ReLU(True),
        )
        self.blocks = nn.Sequential([FFCResBlock(ch) for  in range(nblocks)])

        self.dec = nn.ConvTranspose2d(ch, base_ch, 4, stride=2, padding=1)

        # separate feature heads for T and S before cross-attention
        self.head_T = nn.Sequential(nn.ReLU(True), nn.Conv2d(base_ch, base_ch, 3, padding=1))
        self.head_S = nn.Sequential(nn.ReLU(True), nn.Conv2d(base_ch, base_ch, 3, padding=1))
...
def compute_loss(pred, target, inp, bathy, below, cfg):
    ...
    lp = loss_geostrophic(pred, inp, below)
...
def val_epoch(model, loader, cfg, device):
    ...
    for inp, tgt, bathy, below in loader:
        inp, tgt, bathy, below = _move([inp, tgt, bathy, below], device)
        , parts = computeloss(model(inp), tgt, inp, bathy, below, cfg)
        for k in acc: acc[k] += parts[k]
    n = len(loader)
    return {k: v / n for k, v in acc.items()}
...
def main(cfg=CFG):
    ...
    metrics = run_evaluation(model, val_ds, cfg, device, ep, prefix="final")
    mTL = metrics['T_lama']; mTO = metrics['T_oi']
    mSL = metrics['S_lama']; mSO = metrics['S_oi']
