...
Adopt the corrected LaMa-lite architecture from `bin/todo/model.py` with
...
`model.py` is a clean rewrite: explicit channel split, no tuple passing,
...
### `configs/nc/training/generator/hydro.yaml`
...
### `configs/nc/training/hydro_train.yaml`
...
25 tests in `tests/test_hydro_model.py` covering:
