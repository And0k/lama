...
if os.path.isdir(REPO_DIR) and os.path.isfile(
    os.path.join(REPO_DIR, "saicinpainting", "__init__.py")
):
    pass
    ...
    raise FileNotFoundError(
        "Cannot find the lama repo. Set REPO_DIR or run from repo root."
    )
...
if _resume_ckpt is not None:
    ...
    log.info("  Loaded %d keys, missing=%d, unexpected=%d",
...
log.info("Launch TensorBoard:  %%tensorboard --logdir %s",
         os.path.join(tb_dir, "nc_training"))
